...
class Simhasher {
 ...
 public:
  Simhasher(const string& dictPath = "", const string& modelPath = "",
            const string& user_dict_path = "", const string& idfPath = "",
            const string& stopWords = "")
      : _jieba(dictPath, modelPath, "", idfPath, stopWords) {}
  ~Simhasher(){};

  bool extract(const string& text, vector<pair<string, double> >& res,
               size_t topN) const {
    _jieba.extractor.Extract(text, res, topN);
    return true;
  }
  bool make(const string& text, size_t topN,
            vector<pair<uint64_t, double> >& res) const {
    ...
    for (size_t i = 0; i < res.size(); i++) {
      res[i].first =
          _hasher(wordweights[i].first.c_str(), wordweights[i].first.size(), 0);
  ...
  bool make(const string& text, size_t topN, uint64_t& v64) const {
    ...
    for (size_t i = 0; i < hashvalues.size(); i++) {
      for (size_t j = 0; j < BITS_LENGTH; j++) {
        weights[j] += (((u64_1 << j) & hashvalues[i].first) ? 1 : -1) *
                      hashvalues[i].second;
      }
    }
   ...
   */
  static uint64_t make_from_predefined_keywords(
    ...
    for (size_t i = 0; i < keywords.size(); ++i) {
      hashvalues[i].first =
          _tmp_hasher(keywords[i].first.c_str(), keywords[i].first.size(), 0);
